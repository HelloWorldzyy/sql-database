...
void prepareRecordForTweetMessage(int fieldCount,
		unsigned char *nullFieldsIndicator, const int tweetid, const int userid,
...
void prepareLargeRecordForTweetMessage(int fieldCount,
		unsigned char *nullFieldsIndicator, const int index, void *buffer,
		int *size) {
	int offset = 0;
...
void prepareLargeRecordForTwitterUser(int fieldCount,
		unsigned char *nullFieldsIndicator, const int index, void *buffer,
		int *size) {
	int offset = 0;
...
// Function to prepare the data in the correct form to be inserted/read
void prepareRecordForTwitterUser(int fieldCount,
		unsigned char *nullFieldsIndicator, const int userid,
		const int screen_nameLength, const string &screen_name,
		const int langLength, const string &lang, const int friends_count,
		const int statuses_count, const int usernameLength,
		const string &username, const int followers_count,
...
// Function to prepare the data in the correct form to be inserted/read
void prepareRecord(int fieldCount, unsigned char *nullFieldsIndicator,
		const int nameLength, const string &name, const int age,
		const float height, const int salary, void *buffer, int *recordSize) {
	int offset = 0;
...
void prepareLargeRecord3(int fieldCount, unsigned char *nullFieldsIndicator,
	...
	for (int i = 0; i < 7; i++) {
		...
		nullBit = nullFieldsIndicator[attr_pos_in_nth_byte]
				& (1 << attr_pos_in_nth_bit_in_a_byte);
...
void setAttrNull(void * src,  unsigned short  attrNum, bool isNull)
{
	unsigned bytes = 0;
	unsigned pos = 0;
